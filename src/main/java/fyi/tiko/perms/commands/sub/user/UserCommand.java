                    ...
                    switch (args[1].toLowerCase()) {
                        case "add" -> {
                            ...
                            if (userRepository.hasPermission(uuid, permission)) {
                                translator.sendTranslatedMessage(sender, "commands.user.already-has-permission", name, permission);
                                return;
                            }

                            userRepository.addPermission(uuid, permission);
                            translator.sendTranslatedMessage(sender, "commands.user.added-permission", permission, name);
                        ...
                        case "remove" -> {
                            ...
                            if (!userRepository.hasPermission(uuid, permission)) {
                                translator.sendTranslatedMessage(sender, "commands.user.does-not-have-permission", name, permission);
                                return;
                            }

                            userRepository.removePermission(uuid, permission);
                            translator.sendTranslatedMessage(sender, "commands.user.removed-permission", permission, name);
                    ...
                    if (args[1].equalsIgnoreCase("group")) {
                        switch (args[2].toLowerCase()) {
                            ...
                            case "remove" -> {
                                ...
                                if (!userRepository.isInGroup(uuid, group.name())) {
                                    translator.sendTranslatedMessage(sender, "commands.user.not-in-group", name, group.name());
                                    return;
                                }

                                userRepository.removeGroup(uuid, group.name());
                                translator.sendTranslatedMessage(sender, "commands.user.removed-group", group.name(), name);
