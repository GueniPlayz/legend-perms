        ...
        plugin.runAsync(() -> {
            switch (args[0].toLowerCase()) {
                case "set" -> {
                    ...
                    if (plugin.signs().stream().anyMatch(permissionSign -> permissionSign.location().equals(sign.getLocation()))) {
                    ...
                    plugin.signs().add(permissionSign);
                    translator.sendTranslatedMessage(sender, "commands.sign.set-sign");
                }

                case "remove" -> {
                    if (plugin.signs().stream().noneMatch(permissionSign -> permissionSign.location().equals(sign.getLocation()))) {
                        translator.sendTranslatedMessage(sender, "commands.sign.no-sign-found");
                        return;
                    }

                    var permSign = plugin.signs().stream().filter(permissionSign -> permissionSign.location().equals(sign.getLocation())).findFirst()
                        .orElseThrow();
